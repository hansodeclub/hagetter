...
const EditorStore =
    types.model('EditorModel', {
        ...
        addStatus(status: Status, anchor?: string) {
            ...
            const index = anchor ? self.items.findIndex(item => item.id === anchor) : self.items.length;
            self.items.splice(index === -1 ? 0 : index, 0, {
                id: status.id,
                sortKey: moment(status.created_at).valueOf(),
                selected: false,
                type: 'status',
                data: status
            });
        ...
        addText(text: string, size: string, color: string, anchor?: string) {
            const anchorIndex = anchor ? self.items.findIndex(item => item.id === anchor) : -1;
            const sortKey = anchorIndex !== -1 ? self.items[anchorIndex].sortKey - 1 : generateId();
            self.items.splice(anchorIndex !== -1 ? anchorIndex : self.items.length, 0, {
        ...
        moveItem(direction: 'up' | 'down') {
            ...
            if ((direction === 'up' && self.items[0].selected) ||
